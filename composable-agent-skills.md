...
> **Superseded pending revision.** This plan has been through a five-way adversarial
> review. Several of its facts are wrong and four of its security controls do not hold.
> Read [`REVIEW-FINDINGS.md`](./REVIEW-FINDINGS.md) before acting on anything here.
> Preserved as-reviewed apart from this banner.
...
receiving team updates means giving up your changes. That trade-off is why the repo
has exactly one skill — extension is impossible, so nobody invests in a second.
...
- `.claude/skills/` contains one skill, `git/SKILL.md`, tracked.
- `.claude/commands` is tracked as a **zero-byte regular file** (`100644 e69de29`,
  the empty blob), not a directory. It was introduced by `git add -A` run while
  Claude Code's sandbox bind-mounted `/dev/null` over that path — the same commit
  (`f088f1ff`) zeroed `.claude/settings.json` the same way. It recurred at
  `282cababc` and `5221085f3`; `settings.json` was repaired at `76e6eb547`,
  `commands` never was.
- `.gitignore:241-245` (`# Agents`) covers only `settings.local.json`,
  `agent-memory`, `scheduled_tasks.lock`. The pattern says `.claude/agent-memory`
  but the real directory is `agent-memory-local`, so the committed rule misses;
  only per-machine `.git/info/exclude` currently hides it. A fresh clone gets
  untracked `.claude/` noise immediately.
- `permissions.deny` in `.claude/settings.json` is `[]`.
- There is no `CODEOWNERS` anywhere in the tree, no `CONTRIBUTING.md`, no
  `SECURITY.md`. `azure-pipelines.yml:20` sets `pr: none` — there is no PR gate.
- `core.hooksPath` is `.husky`; `.husky/post-checkout|post-commit|post-merge|pre-push`
  are Git-LFS shims, `pre-commit` runs `lint-staged`. Bootstrap is
  `"postinstall": "husky install"` (`package.json:23`).
- House precedent for generated files is **generated-then-gitignored**, never
  committed: shaders (`.gitignore:84`, with an autogenerated banner stamped into
  the output by `scripts/build-shader.ps1:55-57`), `data-files.generated.ts`,
  `twin-types-bundle.ts`, `Version.ts`.
...
Staleness-check timings in this repo: `git ls-files -s .claude | sha1sum` 4.7–5.6 ms,
...
Templates live outside `.claude/` deliberately: generating files inside a directory
that Claude Code's own sandbox bind-mounts over is exactly how `.claude/commands`
became an empty blob three times.
...
that is literally what happens today. Every output carries an autogenerated banner,
matching `build-shader.ps1:56`.
...
3. **Populate `permissions.deny`** (currently `[]`): `~/.aws/**`, `~/.ssh/**`,
   `~/.claude.json`, `./**/.env`, `~/.config/gcloud/**`. One small PR, and the only
   control that holds regardless of what any skill says — conditional on deny
...
5. **`CODEOWNERS` covering `.claude/**`, `.codex/**`, `.mcp.json`, `.husky/**`,
   `package.json`, and `tools/agent-skills/**`.** Worth doing irrespective of this plan.
...
**Phase 0 — hygiene, independent of everything else.** `git rm --cached .claude/commands`
as its own commit. Fix `.gitignore:243` to `agent-memory*`. Populate `permissions.deny`.
Add `CODEOWNERS`.

**Phase 1 — verify the two unknowns** (below). Both are ~2 minutes and one of them can
change the emitter.

**Phase 2 — compiler and one real skill.** Move `.claude/skills/git/SKILL.md` into
`templates/` and untrack it (it is tracked today; a generator writing into
`.claude/skills/` collides with it on day one). Build `build.ts` + validator +
`explain.ts`. Wire `.claude/settings.json`, `.codex/hooks.json`, and the
`skills:build` script. Add the setup step to `README.md` — it is the only onboarding
doc, there is no `CONTRIBUTING.md`.
...
- **Git hooks as the mechanism** (`post-merge`/`post-checkout`, via the existing
  `.husky/` bootstrap). Rejected on the matrix above: silently misses `git reset --hard`,
...
- **`postinstall` chained after `husky install`.** Matches repo precedent and is already
  proven to run for everyone, but it only fires on `bun install`, so a pulled template
  change sits stale indefinitely. That fails the hard requirement outright. It also runs
  inside four Dockerfiles that call `bun install --frozen-lockfile` without
  `--ignore-scripts`, so it would need to be fail-soft in CI regardless. Kept only as the
...
- **Symlinking `.claude/skills/<name>` to a tracked source dir.** Documented and
  ...
  transformation at all, which is the whole point. Also fragile on Windows, and this
  repo already has evidence of a symlink not surviving.
...
guarantees permanent phantom diffs — the `.claude/commands` failure mode again. This plan
...
inherited. Note `azure-pipelines.yml:20` sets `pr: none`, so there is no PR gate for a
diff check to hang off today anyway.
...
- **opencode.** Installed at `~/.opencode/bin/opencode` but not investigated. It would
  plug in as a third output path and a third registration if it ever matters.
- **`AGENTS.md`.** The repo has one; this plan does not touch it or attempt to generate
  it. Skills and AGENTS.md are different surfaces with different lifecycles.
- **Sharing skills across Twinfinity repos.** `package.json:42` already reaches a sibling
  `../documentation-index/`, so a multi-repo convention may exist. Not addressed.
- **The `.mcp.json` exposure.** `.mcp.json` is tracked, contains a local-command server
  (`playwright-test` → `node_modules/.bin/playwright`), and `.claude/settings.json:26`
  sets `enableAllProjectMcpServers: true` — one added JSON object is unprompted local
  process execution at session start for every developer. Strictly worse than anything a
  skill can do, and pre-existing. It deserves its own ticket; the `CODEOWNERS` control
  above partially covers it.
- **The existing `git` skill's normalisation of `dangerouslyDisableSandbox`.** It
  repeatedly instructs the agent to disable the sandbox and to read the resulting errors
  as noise, which trains the team to click through exactly the prompt a malicious skill
  would need. Worth addressing; not by this plan.
- **The untracked reviewer agents under `.claude/agents/`.** Eight of them are hidden by
  a per-machine `.git/info/exclude:18` while three playwright agents stay tracked. Whether
  they should be shared is a team-process call.
- **Skill-listing token budget.** Every skill's description sits in context every session.
  Not quantified here.

## See Also

- [`.claude/skills/git/SKILL.md`](../../.claude/skills/git/SKILL.md) — the one existing
  skill, and the Phase 2 migration target
- [`scripts/build-shader.ps1`](../../scripts/build-shader.ps1) — the repo's
  generated-then-gitignored precedent, including the autogenerated banner convention
- [`docs/decisions/`](../decisions/) — no ADR currently constrains this; promote one from
  this plan if the trigger and override policy stabilise
